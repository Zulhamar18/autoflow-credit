...
export const getAllowance = async (client, tokenAddress, ownerAddress, spenderAddress) => {
    try {
        const approved = await getActionWithFallback(client, readContract, 'readContract', {
...
export const getAllowanceMulticall = async (client, chainId, tokens, ownerAddress) => {
    if (!tokens.length) {
        return [];
    }
...
export const getTokenAllowance = async (token, ownerAddress, spenderAddress) => {
    ...
    const approved = await getAllowance(client, token.address, ownerAddress, spenderAddress);
    return approved;
};
/**
...
export const getTokenAllowanceMulticall = async (ownerAddress, tokens) => {
    ...
    const allowances = (await Promise.all(chainKeys.map(async (chainId) => {
        ...
        return getAllowanceMulticall(client, chainId, tokenDataByChain[chainId], ownerAddress);
    }))).flat();
    return allowances;
};
